...
evaluate_lagged_cor =
  function(object,
    ...
    if (plot) {
      ...
      # 检查是否有非无穷大且非缺失的值
      non_inf_log_p = temp_data2$log_p[!is.infinite(temp_data2$log_p) & !is.na(temp_data2$log_p)]
      if (length(non_inf_log_p) > 0) {
        temp_data2$log_p[is.infinite(temp_data2$log_p)] =
          max(non_inf_log_p)
      } else {
        temp_data2$log_p[is.infinite(temp_data2$log_p)] = 0
      }
